...
module.exports = function Shared(context) {
	var share = {
		...
		compilerDone: function(stats) {
			...
			const newlyCreatedAssets = stats ? stats.compilation.assets : [];
		   
			fs.readdir(path.resolve(context.options.publicPath), (err, files) => {
			  if(err){
				console.log(err);
				return;	
			  }
			  ...
			  files.forEach(file => {
				if (!newlyCreatedAssets[file] && file.indexOf('hot-update') !== -1) {
				  fs.stat(path.join(context.options.publicPath,file), function (err, stats) {
					 if (err) {
						 return console.error(err);
					 }
				  
					 fs.unlink(path.join(context.options.publicPath,file),function(err){
	...
	if(!context.options.outputToDisk){
